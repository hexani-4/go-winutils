...
func LoadErrIcon() (hIcon uintptr, err error) {
	if procLoadImageW == nil {
		procLoadImageW = user32.MustFindProc("LoadImageW")
	}
...
func ErrorNotifyIcon(message string, duration uint32) (err error) { //broken, need to destroy window after use, make icon exist untill clicked! TODO:
	if procShell_NotifyIconW == nil {
		procShell_NotifyIconW = shell32.MustFindProc("Shell_NotifyIconW")
	}
